...
MOSC_DIR = dirname(dirname(realpath(__file__)))
MOSC_APP = join(MOSC_DIR, 'bin', 'mosct' + args.suffix)

MOSC_APP_WITH_EXT = MOSC_APP
if platform.system() == "Windows":
    MOSC_APP_WITH_EXT += ".exe"

if not isfile(MOSC_APP_WITH_EXT):
    print("The binary file 'mosct' was not found, expected it to be at " + MOSC_APP)
    print("In order to run the tests, you need to build Mosc first!")
    sys.exit(1)

# print("Mosc Test Directory - " + MOSC_DIR)
# print("Mosc Test App - " + MOSC_APP)
        ...
        with open(self.path, 'r', encoding="utf-8", newline='', errors='replace') as file:
            ...
            for line in lines:
                ...
                if match:
                    ...
                    # If we expect a compile error, it should exit with MSC_TERR_COMPILATION.
                    self.exit_code = 65
                    expectations += 1
                ...
                if match:
                    ...
                    # If the runtime error isn't handled, it should exit with MSC_TERR_SOFTWARE.
...
def run_script(app, path, type):
    ...
    if (splitext(path)[1] != '.msc'):
        return

    # Check if we are just running a subset of the tests.
    if args.suite:
        this_test = relpath(path, join(MOSC_DIR, 'test'))
        if not this_test.startswith(args.suite):
            return

    # Update the status line.
    print_line('({}) Passed: {} Failed: {} Skipped: {} '.format(
        relpath(app, MOSC_DIR), green(passed), red(failed), yellow(num_skipped)))

    # Make a nice short path relative to the working directory.

    # Normalize it to use "/" since, among other things, mosc expects its argument
    # to use that.
...
def run_test(path, example=False):
    run_script(MOSC_APP, path, "test")


def run_api_test(path):
    run_script(MOSC_APP, path, "api test")
...
def run_example(path):
    ...
    run_script(MOSC_APP, path, "example")


walk(join(MOSC_DIR, 'test'), run_test, ignored=['api', 'benchmark', "examples"])
walk(join(MOSC_DIR, 'test', 'api'), run_api_test)
# walk(join(MOSC_DIR, 'example'), run_example)
